...
func PublishList(ctx context.Context, rc *app.RequestContext) {
	token := rc.GetString("token")

	// token authentication
	uid, err := strconv.ParseInt(rc.Query("user_id"), 10, 64)
	if err != nil {
		rc.JSON(http.StatusOK, response.PublishList{
			Base: response.Base{
				StatusCode: -1,
